...
@ContextConfiguration(classes=[ParentChangeMicroserviceTestConfig])
class ParentChangeMicroServiceSpec extends Specification {
	@Autowired
	ParentChangeMicroService underTest;
...
class ParentChangeMicroserviceTestConfig {
	...
	@Bean
	ParentChangeMicroService underTest() {
		return new ParentChangeMicroService()
	}
	@Bean
	WorkManagementService workManagementService() {
