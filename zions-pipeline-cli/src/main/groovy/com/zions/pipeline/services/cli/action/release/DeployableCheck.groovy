...
class DeployableCheck implements CliAction {
	...
	@Value('${xl.password:4878Middy004}')
	String xlPassword
...
//				packageName = packageName.substring(packageName.indexOf('_')+1)
...
//		}
	...
	def createPackage(String packageName, outDir) {
		...
		new AntBuilder().exec(dir: "${xlwLocation}", executable: 'cmd') {
		...
		if (createRelease) {
			...
			new AntBuilder().exec(dir: "${xlwLocation}", executable: 'cmd') {
				arg( line: "/c xlw.bat apply -f xl-create-release.yaml --xl-release-url ${xlrUrl} --xl-release-username ${xlUser} --xl-release-password ${xlPassword} --values app_version=${packageName},release_name=\"UOB-${packageName}\"")
			}
